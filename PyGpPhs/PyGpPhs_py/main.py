...
def main():
    ...
    plt.plot(np.arange(0, Result_num_data, 0.1), model.get_dX()[0, :], color='blue', label='PyGpPhs_py computed dX', linewidth=1)
    plt.xlabel('Time (s)')
    ...
    plt.plot(np.arange(0, Result_num_data, 0.1), model.get_dX()[1, :], color='blue', label='PyGpPhs_py computed dX', linewidth=1)
    plt.xlabel('Time (s)')
    plt.ylabel('Force (N)')
    plt.legend()
    # plt.show()
    ...
    surf2 = ax.plot_surface(X_test, Y_test, np.reshape(pred, n_test), color='gold',linewidth=1, label='PyGpPhs_py Hamiltonian', alpha=1)
